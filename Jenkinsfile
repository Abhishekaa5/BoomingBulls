pipeline {
    ...
    stages {
        ...
        stage('Build Docker') {
            steps {
                sh 'sudo docker build -t devops-app ./app'
            }
        }
        ...
        stage('Push Image') {
            steps {
                sh '''
                sudo docker tag devops-app:latest $ECR_REPO:latest
        ...
        stage('Deploy to EC2') {
            steps {
                ...
                ssh -o StrictHostKeyChecking=no ec2-user@$EC2_IP << EOF
                  ...
                  sudo docker run -d -p 80:80 --name app --restart always $ECR_REPO:latest
                EOF
                '''
            }
        }
